        ...
        if (pick) {
          ...
          if (isMegaClearingRow(pickRow)) {
            ...
            if (mz >= 0 && !megaClearedZones[mz]) {
              ...
              // ── Spawn Queen Gate at the bottom of the mega-clearing ──
            ...
            // Check if all queens for this zone are dead → passage opened
            if (megaClearedZones[mz] && !megaClearedZones[mz + '_gateOpen']) {
              var aliveQueens = 0;
              for (var qci = 0; qci < bats.length; qci++) {
                if (bats[qci].isQueen && bats[qci].zone === mz && bats[qci].state !== 'dead') aliveQueens++;
              }
              if (aliveQueens === 0) {
    ...
    function checkPickZombieCollisions() {
      ...
      var ZR = ZOMBIE_SIZE * 0.45; // zombie collision half-size
      var allPicks = [pick].concat(userPicks, extraPicks);
      for (var pi = 0; pi < allPicks.length; pi++) {
        var p = allPicks[pi];
        if (!p || p.stuck) continue;
    ...
    function drawZombies() {
      for (var i = 0; i < zombies.length; i++) {
        ...
        if (z.state !== 'dead' && z.hp < z.maxHp) {
          ...
          ctx.fillStyle = '#333'; ctx.fillRect(barX, barY, barW, barH);
          ctx.fillStyle = '#44ff44'; ctx.fillRect(barX, barY, barW * (z.hp / z.maxHp), barH);
        }
